...
module.exports = function() {
    ...
    this.minerPath = './miners/cpuminer-opt';
    ...
    this.getSummary = function() {
        ...
        client.on('data', (data) => {
            ...
            this.accepted = this.summary.acc;
            this.rejected = this.summary.rej;
        });
    ...
    this.getHashrate = function() {
        ...
        client.on('data', (data) => {
            ...
            this.hashrate = parseFloat(this.hashrate);
        });
    ...
    this.configure = function(algo, threads, stratumUrl, stratumUser, stratumPass) {
        if (this.algoSupported.indexOf(algo) > -1) {
            ...
            console.log('cpuminer-opt: Algo "' + algo + '" is not supported by this miner.' );
            return false;
        }
        ...
        if (threads >= 1) {
            ...
            console.log('cpuminer-opt: Threads needs to be a positive number.');
            return false;
        }
    ...
    this.run = function () {
        if ( this.configured !== true ) {
            console.log('cpuminer-opt Not configured yet!');
            return;
        }

        if ( this.status == 'running') {
            console.log('cpuminer-opt: Already running!');
        }
        ...
        if ( !miner ) {
            this.status = 'error';
            console.log('cpuminer-opt: Unhandled error occured.');
        }
